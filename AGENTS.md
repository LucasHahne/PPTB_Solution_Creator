...
**Solution Creator** is a Power Platform ToolBox tool for designing and deploying
Dataverse schema (solutions, tables, columns, lookups) faster than the maker
portal. Stack: React 18, TypeScript, Vite, Zustand, TanStack Table, Tailwind 3,
published as `@lucas001-yt/pptb-solution-creator`.

This repo also uses the **AI Blueprint** workflow overlay (skills + plans). Never
run a framework scaffolder inside a directory that already holds blueprint files
(`AGENTS.md`, `CLAUDE.md`, `.agents/`, `.claude/`, `blueprint/`).
...
- Dev server: `npm run dev` (Vite; use ToolBox local debug for full API behavior)
- Build: `npm run build` (`tsc && vite build` → `dist/`)
- Preview build: `npm run preview`
- Watch build: `npm run watch` / `npm run dev-watch`
- Validate ToolBox package: `npm run validate` (`pptb-validate`)
- Finalize npm package: `npm run finalize-package` (refresh `npm-shrinkwrap.json`)

No unit test command is configured yet. Testing is opt-in: run `/tests` or
`$tests` to add a runner and declare a `test` command here.

No lint script is configured.

No combined `Verify` command or GitHub Actions workflows yet. Run `/ci` or `$ci`
when you want automatic checks.
