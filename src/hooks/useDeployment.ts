  ...
  const run = useCallback(
    async (project: SolutionProject) => {
      ...
      if (result.status === 'success') {
        appendLog(
          'success',
          `Done. Created ${result.createdTables} table(s), ${result.createdColumns} column(s), ${result.createdRelationships} lookup(s), ${result.createdBridgeLookups} M:N link(s), ${result.createdGlobalChoices} global choice(s).`,
        );
        await clearDraft();
